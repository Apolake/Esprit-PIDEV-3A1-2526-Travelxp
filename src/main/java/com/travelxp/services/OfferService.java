...
public class OfferService {
    ...
    /**
     * Return all offers belonging to a given property.
     */
    public List<Offer> getOffersByPropertyId(Long propertyId) throws SQLException {
        List<Offer> offers = new ArrayList<>();
        String sql = "SELECT * FROM offer WHERE property_id = ?";
        try (PreparedStatement ps = cnx.prepareStatement(sql)) {
    ...
    private Offer mapResultSetToOffer(ResultSet rs) throws SQLException {
        ...
        Offer offer = new Offer(id, propertyId, title, description, discountPercentage, startDate, endDate, isActive, createdAt);
        return offer;
    }
}
